...
$this->createTable('{{%location}}', [
    ...
    'age' => $this->integer()->append('CHECK (age >= 18 AND age <= 99)'),
...
return RuleBuilder::rules([
    Attribute::create('name')->string()->notNull(),
    Attribute::create('age')->integer()->min(18)->max(99),
    Attribute::create('status')->string()->defaultValue('active')->in(['active', 'inactive', 'deleted']),
]);
```
...
## Gii Integration (Automated Code Generation)

The package includes a built-in Gii Template and an automatic parsing utility (GiiHelper) that intercepts the native multi-dimensional array layouts generated by Yii2's Gii engine and seamlessly compiles them into clean, fluent architecture.

### 1. Enable the Template in your Application

Register the custom model template within your application configuration file (usually config/main-local.php or config/web.php under the development environment check):
